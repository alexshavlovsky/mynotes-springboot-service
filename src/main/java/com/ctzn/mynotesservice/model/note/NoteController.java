...
public class NoteController {
    ...
    @GetMapping()
    public List<NoteResponse> getAllNotesByNotebook(@RequestParam(required = false) Long nbId) throws EntityByIdNotFoundException {
        if (nbId == null) {
            return domainMapper.mapAll(noteRepository.findAll(), NoteResponse.class);
        } else {
            // TODO: create tests for this case
            NotebookEntity notebookEntity = notebookRepository.findById(nbId).orElseThrow(() -> new EntityByIdNotFoundException("Notebook", nbId));
    ...
    public NoteResponse getNote(
            ...
            @PathVariable("id") NoteEntity noteEntity) throws EntityByIdNotFoundException {
        if (noteEntity == null) throw new EntityByIdNotFoundException("Note", id);
        return domainMapper.map(noteEntity, NoteResponse.class);
    }

    @PostMapping() // create only
    @ResponseStatus(HttpStatus.CREATED)
    public NoteResponse saveNote(@RequestBody NoteRequest noteRequest) throws EntityByIdNotFoundException {
    ...
    public NoteResponse updateNote(
        ...
        if (noteEntity == null) throw new EntityByIdNotFoundException("Note", id);
        long nbId = noteRequest.getNotebookId();
        NotebookEntity requestNotebookEntity = notebookRepository.findById(nbId).orElseThrow(() -> new EntityByIdNotFoundException("Notebook", nbId));
